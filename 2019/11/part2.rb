...
def turn(val)
  ...
  $x += Math.cos($dir * Math::PI / 2).round
  $y += Math.sin($dir * Math::PI / 2).round
end
...
while (mem[pos] != 99)
  ...
  when 2
    ...
    pos += 4
  when 3
    set_mem(mem, address_p1, grid[[$x,$y]])
    pos += 2
  when 4
    if paint
      grid[[$x,$y]] = p1
...
width = grid.keys.map(&:first).minmax
height = grid.keys.map(&:last).minmax

Range.new(*height).each do |y|
  Range.new(*width).each do |x|
    print(grid[[x,y]] == 1 ? '#' : ' ')
  end
  puts
end
