...
(4..inventory.length).each do |num_drops|
  inventory.combination(num_drops).each do |to_drop|
    drop_commands = to_drop.map { |str| "drop #{str}\n" }
    puts "Trying #{drop_commands.inspect}"
    result = run(program + drop_commands.join + "north\n")
    if !result
      puts "failed with #{drop_commands.inspect}"
      next
    end
